 ...
 *
 * DSPy-style signature for presentation slides generation.
 ...
 */
const SLIDES_INSTRUCTIONS = `You are an expert presentation designer. Create a professional presentation based on the user's request and the provided research content.
...
export const SlidesSignature = createSignature('SlidesSignature')
  ...
  .input('researchContent', FieldType.STRING, {
    description: 'Research content to base the presentation on',
    required: true
  })
  .output('presentation', FieldType.OBJECT, {
    description: 'Structured slides data',
    required: true,
    properties: slidesSchema.properties
  })
  .build();

export function generateSlidesSignaturePrompt(userPrompt, researchContent) {
...
export function validateSlidesInputs(userPrompt, researchContent) {
